...
const calcQuoteRegex = /POST_OPTIONS\.forEach\(p=>\{[\s\S]*?\}\);/g;

// Only replace the FIRST match inside calcQuote
let matchCount = 0;
html = html.replace(calcQuoteRegex, (match) => {
  matchCount++;
  if (matchCount === 1) { // calcQuote loop
    return `POST_OPTIONS.forEach(p=>{
...
console.log('calcQuote loop fixed');
